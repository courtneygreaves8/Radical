  ...
  return (
    ...
    >
      <Link
        to={href}
        className={cn(
          'relative block overflow-hidden border border-white/10 bg-white/5',
      ...
      >
        <div
          className={cn(
            'photo-grain relative overflow-hidden',
        ...
        >
          ...
          <div className="absolute inset-0 z-[3] flex items-center justify-center opacity-0 transition group-hover:opacity-100">
            <span className="flex size-12 items-center justify-center bg-lime text-ink">
              <Play className="size-5 fill-current" />
            </span>
          </div>
          <span className="absolute bottom-2 right-2 z-[3] bg-black/80 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-white/90">
            {episode.duration}
          </span>
        </div>
...
export function ShowPoster({ show }: ShowPosterProps) {
  return (
    ...
    >
      ...
      >
        <div className="aspect-[2/3] overflow-hidden photo-grain">
          <img
            src={show.cover}
            alt=""
            className="photo-bw size-full object-cover transition duration-500 group-hover:scale-105"
